...
//        list.forEach(s -> System.out.println(s.getId()));
    ...
    @Operation(summary = "Check staff username availability")
    @GetMapping(value = "/exists", produces = "application/json; charset=UTF-8")
    public ResponseEntity<ApiResponse<Boolean>> checkUsername(@RequestParam String username) {
        boolean available = staffService.isUsernameAvailable(username);
        return ResponseEntity.ok(new ApiResponse<Boolean>().ok(available));
    }
